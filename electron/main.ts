...
function toNetscapeCookieLine(cookie: any): string {
  ...
  const expires = cookie.expires && cookie.expires > 0 ? Math.floor(cookie.expires) : Math.floor(Date.now() / 1000) + 86400
  const prefix = cookie.httpOnly ? '#HttpOnly_' : ''
  return `${prefix}${domain}\t${includeSubdomains}\t${pathValue}\t${secure}\t${expires}\t${cookie.name}\t${cookie.value}`
}

async function createDouyinFreshCookiesFile(url: string): Promise<string | null> {
  ...
  try {
    ...
    const cookies = await page.cookies('https://www.douyin.com/', url)
    if (!cookies.some((cookie: any) => cookie.name === 's_v_web_id')) return null

    const cookieFile = path.join(app.getPath('temp'), `ybdown-douyin-${Date.now()}-${Math.random().toString(16).slice(2)}.cookies.txt`)
    const content = [
      '# Netscape HTTP Cookie File',
      '# Generated by YBDown for Douyin metadata extraction.',
...
async function parseDouyinWithPuppeteer(url: string): Promise<any> {
  ...
  try {
    ...
    page.on('response', async (response) => {
      ...
      if (contentType.startsWith('video/') && !resUrl.includes('douyin-pc-web')) {
    ...
    if (detailData) {
      ...
      const mediaUrl = mediaUrls[mediaUrls.length - 1]
      videoInfo = {
        id: url.match(/\/video\/(\d+)/)?.[1] || Date.now().toString(),
        title: title.replace(/\s*-\s*抖音\s*$/, '') || '抖音视频',
        description: title.replace(/\s*-\s*抖音\s*$/, '') || '',
        thumbnail: '',
        duration: 0,
        uploader: '',
        webpageUrl: page.url(),
        formats: [{
          formatId: 'browser_media',
          quality: '默认',
          ext: 'mp4',
          filesize: 0,
          width: 0,
...
ipcMain.handle('ytdlp:parse', async (_event, ...args) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!cookiesFile && isDouyinUrl(url)) {
      try {
        sendProcessLog({ scope: 'parse', message: '正在为抖音生成临时访客 Cookie' })
        generatedCookiesFile = await createDouyinFreshCookiesFile(url) || ''
        if (generatedCookiesFile) {
          args.push('--cookies', generatedCookiesFile)
          sendProcessLog({ scope: 'parse', message: '已生成抖音临时 Cookie，继续提取元数据' })
        } else {
          sendProcessLog({ scope: 'parse', level: 'warn', message: '未生成抖音临时 Cookie，继续使用普通解析' })
          appendCookieArgs(args, url, cookiesFile, { includeBrowser: false })
        }
      } catch (e: any) {
        sendProcessLog({ scope: 'parse', level: 'warn', message: `生成抖音临时 Cookie 失败：${e.message || e}` })
        appendCookieArgs(args, url, cookiesFile, { includeBrowser: false })
      }
    } else {
      appendCookieArgs(args, url, cookiesFile, { includeBrowser: false })
    ...
    } else if (generatedCookiesFile) {
      sendProcessLog({ scope: 'parse', message: '使用自动生成的抖音临时 cookies.txt' })
    } else {
    ...
    try {
      ...
      if (code !== 0) {
        if (generatedCookiesFile) {
          fs.rm(generatedCookiesFile, { force: true }, () => {})
        }
